  ...
  if (current !== null) {
    ...
    if (
      ...
      if (!checkScheduledUpdateOrContext(current, renderLanes)) {
        ...
        console.log('%c [  ]-597', 'font-size:13px; background:pink; color:#bf2c9f;', )
  ...
  if (enableLazyContextPropagation) {
    ...
    if (oldProps !== null) {
      ...
      if (is(oldValue, newValue)) {
        ...
        // value没有发生变化，如果children也没有发生变化，不需要再进行后面的render流程，直接bailout
        if (
        ...
        // them to update.
        // context value 已更改。搜索匹配的后代消费者并调度它们进行更新。
...
function bailoutOnAlreadyFinishedWork(
  ...
  // 检查子节点们是否有pending work
  if (!includesSomeLane(renderLanes, workInProgress.childLanes)) {
    // ! 1. 子节点们没有pending work，直接跳过就行，即返回null~
  ...
  // ! 2. 当前fiber没有pending work，但是子节点们有。这个时候就只clone一份child fibers，然后返回child。
