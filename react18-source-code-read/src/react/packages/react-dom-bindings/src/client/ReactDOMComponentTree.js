...
export function getClosestInstanceFromNode(targetNode: Node): null | Fiber {
  ...
  // 如果直接事件目标不是 React 拥有的 DOM 节点，我们需要查看其父级中是否有一个是 React 拥有的 DOM 节点。
  ...
  while (parentNode) {
    ...
    // 检查这是否是一个可能在将来包含 React 节点的container root。
    // 我们需要首先检查这一点，因为如果我们是一个脱水容器的子级，我们需要先找到该内部容器，然后再找到父实例。请注意，我们不在 targetNode 本身上检查这个字段，因为 fiber 概念上位于容器节点和第一个子节点之间。它并不围绕容器节点。如果不是容器，我们将检查它是否是一个实例。
    targetInst =
