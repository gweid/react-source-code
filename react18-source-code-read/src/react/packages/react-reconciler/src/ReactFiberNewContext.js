...
export function scheduleContextWorkOnParentPath(
  ...
  // 更新所有祖先的childLanes，包括alternates。
  let node = parent;
  while (node !== null) {
...
function propagateContextChange_eager<T>(
  ...
  while (fiber !== null) {
    ...
    if (list !== null) {
      ...
      // 如果有context消费，那么接下来遍历context单链表，找到匹配的context，然后标记fiber的lanes
      while (dependency !== null) {
        ...
        if (dependency.context === context) {
          ...
          if (fiber.tag === ClassComponent) {
            // 如果是类组件，那么按照类组件的更新逻辑。因为类组件的更新逻辑与其它组件不同。
      ...
      // 如果这一个一个匹配的provider，那么不需要继续遍历。因为Provider会再次走到updateContextProvider
