...
function appendAllChildren(
  ...
  if (supportsMutation) {
    ...
    while (node !== null) {
      // 如果节点是原生标签或者文本节点，那么node.stateNode就是真实的dom节点，直接添加到parent这个父dom节点中即可
      if (node.tag === HostComponent || node.tag === HostText) {
        ...
        // 如果node这个fiber本身不直接对应DOM节点，那么就往下找它的子节点，直到找到有真实的DOM节点的fiber为止
    ...
    case HostComponent: {
      ...
      if (current !== null && workInProgress.stateNode != null) {
        ...
        if (wasHydrated) {
          ...
          // ! sy
          const rootContainerInstance = getRootHostContainer();
          // 在ReactFiberConfigDOM.js中，createInstance创建的DOM节点
          // 还在dom节点上存储了fiber和props
