...
// render应该花费多长时间，然后我们开始遵循 CPU 悬挂启发式并选择退出渲染更多内容。
...
export function requestDeferredLane(): Lane {
  if (workInProgressDeferredLane === NoLane) {
    ...
    //! 如果在同一次渲染中有多个 useDeferredValue 钩子，它们生成的任务应该被一起批处理，因此它们应该都接收相同的 lane。
    ...
    // OffscreenLane 用于预渲染，但我们还将 OffscreenLane 用于增量hydration。它被赋予最低的优先级，
    // 因为初始 HTML 与最终 UI 相同。但在hydration期间使用 useDeferredValue 是一个例外 — 我们需要将 UI 升级到最终值。
    // 因此，如果我们当前正在进行hydration，我们将其视为一个过渡。
  ...
  // 标记父级Suspense边界，以便它知道要生成延迟的lane。
...
export function performConcurrentWorkOnRoot(
  ...
  console.log('%c [ performConcurrentWorkOnRoot ]-917', 'font-size:13px; background:blue; color:#8a7c7c;', ) //sy-log
...
export function performSyncWorkOnRoot(root: FiberRoot, lanes: Lanes): null {
  console.log('%c [ performSyncWorkOnRoot ]-1423', 'font-size:13px; background:blue; color:white;', )
  if ((executionContext & (RenderContext | CommitContext)) !== NoContext) {
    throw new Error('Should not already be working.');
  }
...
function workLoopSync() {
  // console.log('%c [ workLoopSync ]-2130', 'font-size:13px; background:pink; color:#bf2c9f;', )
...
function renderRootConcurrent(root: FiberRoot, lanes: Lanes) {
  console.log('%c [ renderRootConcurrent ]-101', 'font-size:13px; background:red; color:black;', )
  const prevExecutionContext = executionContext;
  // ! 1. 记录 render阶段 开始
  executionContext |= RenderContext;
  const prevDispatcher = pushDispatcher(root.containerInfo);
...
function performUnitOfWork(unitOfWork: Fiber): void {
  // console.log('%c [ performUnitOfWork ]-2399', 'font-size:13px; background:pink; color:#bf2c9f;')
...
function commitRootImpl(
  ...
  console.log('%c [commitRootImpl  ]-2781', 'font-size:13px; background:yellow; color:000;', )
  do {
  ...
  if (
    ...
    if (!rootDoesHavePassiveEffects) {
      console.log('%c [  ]-flushPassiveEffects', 'font-size:13px; background:pink; color:#bf2c9f;', rootDoesHavePassiveEffects)
...
function flushPassiveEffectsImpl() {
  ...
  // console.log('%c [ commit phase flushPassiveEffectsImpl ]-3345', 'font-size:13px; background:pink; color:#bf2c9f;', )
  executionContext |= CommitContext;
